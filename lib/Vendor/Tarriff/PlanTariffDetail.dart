...
import 'package:tankcare/VendorModels/(ListJson)VendorTariffPropertyType.dart';
import 'package:tankcare/VendorModels/PlanTariffPlanDetails.dart';
import 'package:tankcare/string_values.dart';

class VendorPlanTariffDetail extends StatefulWidget {
  VendorPlanTariffDetail({Key key, this.planid});

  String planid;

  @override
  VendorPlanTariffDetailState createState() => VendorPlanTariffDetailState();
}

class VendorPlanTariffDetailState extends State<VendorPlanTariffDetail> {
  bool loading = false;
  PlanTariffDetailModel li;

  var _kGooglePlex;

  VendorTariffPropertyDetailModelList li2;
  ...
  Future<http.Response> details(planid) async {
    ...
    if (response.statusCode == 200) {
      ...
      li = PlanTariffDetailModel.fromJson(json.decode(response.body));
      setState(() {
        PlanController.text = li.res.planDatas.planName.toString();
        ServiceController.text = li.res.planDatas.planServicetype.toString();
        PropertyTypeController.text =
            li.res.planDatas.planPropertytypeId.toString();
  ...
  Future<http.Response> propertytype() async {
    setState(() {
      loading = true;
    });
    ...
    if (response.statusCode == 200) {
      setState(() {
        loading = false;
        li2 = VendorTariffPropertyDetailModelList.fromJson(
            jsonDecode(response.body));

        for (int i = 0; i < li2.tariff.length; i++)
          if (li2.tariff[i].ptypeId == li.res.planDatas.planPropertytypeId)
            setState(() {
              PropertyTypeController.text = li2.tariff[i].ptypeName;
            });
  ...
  TextEditingController LiterToController = new TextEditingController();
  ...
  TextEditingController LiterFromController = new TextEditingController();
...
//  TextEditingController SizeRangeControllermax = new TextEditingController();
  ...
  void initState() {
    check().then((value) {
      if (value)
        details(widget.planid).then((value) => propertytype());
      else
        showDialog<void>(
          context: context,
                      ...
                      child: ExpansionTile(
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: new TextField(
                                enabled: false,
                                controller: LiterFromController,
                                decoration: InputDecoration(
                                  labelText: 'Value From',
                                  hintStyle: TextStyle(
                                    color: Colors.grey,
                                    fontSize: 16.0,
                                  ),
