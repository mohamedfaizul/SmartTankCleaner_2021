...
import 'VendorServiceDetail.dart';

class VendorTommorrowServiceList extends StatefulWidget {
  @override
  VendorTommorrowServiceListState createState() => VendorTommorrowServiceListState();
}

class VendorTommorrowServiceListState extends State<VendorTommorrowServiceList> {
  bool loading = false;
  ...
  Future<http.Response> postRequest() async {
    ...
    if (dropdownValue == '-- Approval Status --')
      url = String_values.base_url +
          'property-plan-services?page=${page}&limit=10&from_date=${dateFormatter.format(date)}&to_date=${dateFormatter.format(date)}';
    else
      url = String_values.base_url +
          'property-plan-services?status=${dropdownValue.toUpperCase()}&page=${page}&limit=10&from_date=${dateFormatter.format(date)}&to_date=${dateFormatter.format(date)}';
    print(url);
                              ...
                              child: Wrap(
                                  ...
                                  children: [
                                    Text(DateFormat('dd/MM/yyyy').format(
                                        DateTime.parse(list.pserviceDate)),
                                        textAlign: TextAlign.center)
                                  ]))),
                    ),
                    DataCell(
