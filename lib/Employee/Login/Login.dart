...
class EmployeeLoginPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: EmployeeLoginPages(),
    );
  }
}

class EmployeeLoginPages extends StatefulWidget {
  @override
  EmployeeLoginPagesState createState() => EmployeeLoginPagesState();
}

class EmployeeLoginPagesState extends State<EmployeeLoginPages> {
  bool _isHidden = true;
  ...
  Future<bool> setRegistered(userid, token, email, userrole) async {
  ...
  Future<int> uploadImage(url) async {
    ...
    response.stream.transform(utf8.decoder).listen((value) {
      ...
      if (li.status) {
        ...
        MyApp.role = li.uroleName;
        setRegistered(li.uid, li.token, emailController.text, li.uroleName);
        RegisterPagesState.cus_id = li.uid;
        RegisterPagesState.token = li.token;
        // if (li.uroleName == "SUPERVISOR")
          Navigator.pushReplacement(context,
              MaterialPageRoute(builder: (context) => EmployeeDashboard(firsttime: true,)));
        // else if (li.uroleName == "VENDOR")
        //   Navigator.pushReplacement(context,
