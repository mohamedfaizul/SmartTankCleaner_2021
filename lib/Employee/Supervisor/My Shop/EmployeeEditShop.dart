...
class EmployeeEditRentalShop extends StatefulWidget {
  EmployeeEditRentalShop({Key key, this.id});

  String id;

  @override
  EmployeeEditRentalShopState createState() => EmployeeEditRentalShopState();
}

class EmployeeEditRentalShopState extends State<EmployeeEditRentalShop> {
  ...
  Future<http.Response> details(planid) async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        shopid = li5.values.shopUid.toString();
        shopusertype = li5.values.shopUtype.toString();
        sql_dob_from = li5.values.shopCStartDate.toString();
        ...
        datefromcontroller.text =   DateFormat('dd/MM/yyyy').format(
            DateTime.parse(li5.values.shopCStartDate.toString()));
        ContractYearsController.text = li5.values.shopCYears.toString();
        PincodeController.text = li5.values.shopPincode.toString();
        dropdownValue1 = li5.values.stateName.toString();
        districtRequest(li5.values.stateId).then((value) {
          setState(() {
            dropdownValue2 = li5.values.districtName.toString();
          });
        });
        statetype = int.parse(li5.values.stateId);
        districttype = int.parse(li5.values.districtId);
      });
...
//    print("response: ${PlanServiceYearClass.toJson()}");
  ...
  Widget build(BuildContext context) {
    Future<int> uploadImage() async {
      ...
      request.fields['shop_id'] = widget.id;
      request.fields['shop_utype'] = shopusertype;
                          ...
                          child: FlatButton(
                            onPressed: () {
                              if (PincodeController.text.length == 6 &&
                                  ...
                                  ContractYearsController.text.length != 0 &&
                                  _typeAheadController.text.length != 0 &&
                                  ShopNameController.text.length != 0 &&
                                  ShopRentController.text.length != 0 &&
                                  ShopDepositController.text.length != 0 &&
                                  OwnerNameController.text.length != 0 &&
                                  OwnerPhoneController.text.length > 9 &&
                                ...
                                if (ShopNameController.text.length == 0)
                                  showDialog<void>(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        title:
                                            Text("Shop Name cannot be empty"),
                                        actions: <Widget>[
                                          TextButton(
                                            child: Text('OK'),
                                            onPressed: () {
                                ...
                                else if (_typeAheadController.text.length == 0)
                                  showDialog<void>(
                                    context: context,
                                    barrierDismissible: false,
                                    // user must tap button!
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        title: Text("Shop For cannot be empty"),
                                        actions: <Widget>[
                                          TextButton(
                                            child: Text('OK'),
                                            onPressed: () {
                                  ...
                                  showDialog<void>(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        title:
                                            Text("Shop Rent Cannot be Empty"),
                                        actions: <Widget>[
                                          TextButton(
                                            child: Text('OK'),
                                            onPressed: () {
                                  ...
                                  showDialog<void>(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        title: Text(
                                            "Owner Phone Number should be 10 characters"),
                                        actions: <Widget>[
                                          TextButton(
                                            child: Text('OK'),
                                            onPressed: () {
