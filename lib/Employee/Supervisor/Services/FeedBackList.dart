...
import 'package:tankcare/Employee/EmployeeModels/FeedBack/FeedbackList.dart';
...
class EmployeeFeedbackList extends StatefulWidget {
  @override
  EmployeeFeedbackListState createState() => EmployeeFeedbackListState();
}

class EmployeeFeedbackListState extends State<EmployeeFeedbackList> {
  bool loading = false;
  final TextEditingController _typeAheadController = TextEditingController();
  FeedBackListModel li;
  ...
  Future<http.Response> postRequest() async {
    ...
    if (dropdownValue == '-- Approval Status --')
      url = String_values.base_url + 'service-feedback-list';
    else
      url = String_values.base_url +
          'service-feedback-list?search=${searchController.text}&page=${page}&limit=10';
    ...
    if (response.statusCode == 200) {
      ...
      li = FeedBackListModel.fromJson(json.decode(response.body));
    } else {
      setState(() {
        loading = false;
      });
        ...
        for (int i = 0; i < li3.list.length; i++)
          if (li3.list[i].title == "Services") {
            for (int j = 0; j < li3.list[i].children.length; j++)
              if (li3.list[i].children[j].title == "Feedback") {
