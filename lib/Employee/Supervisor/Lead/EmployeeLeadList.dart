...
import 'package:tankcare/Employee/EmployeeModels/Lead/LeadList.dart';
import 'package:tankcare/Employee/Supervisor/Lead/EmployeeLeadAdd.dart';
import 'package:tankcare/Employee/Supervisor/Lead/EmployeeLeadDetail.dart';
import 'package:tankcare/Employee/Supervisor/Lead/EmployeeLeadEdit.dart';
import 'package:tankcare/VendorModels/MenuModel.dart';
import 'package:tankcare/string_values.dart';

class EmployeeLeadList extends StatefulWidget {
  @override
  EmployeeLeadListState createState() => EmployeeLeadListState();
}

class EmployeeLeadListState extends State<EmployeeLeadList> {
  bool loading = false;

  EmployeeLeadListModel li;
  ...
  Future<http.Response> postRequest() async {
    ...
    var url = String_values.base_url +
        'lead-list?search=${searchController.text}&page=${page}&limit=10';
    ...
    if (response.statusCode == 200) {
      ...
      li = EmployeeLeadListModel.fromJson(json.decode(response.body));
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        for (int i = 0; i < li3.list.length; i++)
          if (li3.list[i].title == "Lead "||li3.list[i].title == "Lead") {
            if (li3.list[i].children.toString() == "null") {
              ...
              for (int j = 0; j < li3.list[i].children.length; j++)
                if (li3.list[i].children[j].title == "Lead List") {
  ...
  Future<http.Response> delete(planid) async {
    ...
    var url = String_values.base_url + 'lead-delete/' + planid;
    print(String_values.base_url);
    var response = await http.get(
      url,
      headers: {
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: FlatButton(
                            onPressed: () {
                              Navigator.push(context,MaterialPageRoute(builder: (context) => EmployeeLeadAdd()));
                            },
                            child: Text(
                              "Add Lead",
                                ...
                                Center(
                                    child: Center(
                                        child: Wrap(
                                            ...
                                            alignment: WrapAlignment.center,
                                            children: [
                                      Text(list.leadFollowUtype.toString(),
                                          textAlign: TextAlign.center)
                                    ]))),
                              ),
                              DataCell(
                                      ...
                                      setState(() {
                                        if (newValue == "View")
                                          ...
                                          Navigator.pushReplacement(
                                              ...
                                              MaterialPageRoute(
                                                  builder: (context) =>
                                                      EmployeeLeadEdit(id: list.leadId)));
