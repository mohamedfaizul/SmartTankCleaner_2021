...
class EmployeeCustomerEdit extends StatefulWidget {
  EmployeeCustomerEdit({Key key, this.id});

  String id;

  @override
  EmployeeCustomerEditState createState() => EmployeeCustomerEditState();
}

class EmployeeCustomerEditState extends State<EmployeeCustomerEdit> {
  ...
  Future<http.Response> details(planid) async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        dropdownValue1 = li5.items.stateName;
        districttype = int.parse(li5.items.cusDistrict);
        statetype = int.parse(li5.items.cusState);
  ...
  void initState() {
    stateRequest()
        .then((value) => details(widget.id))
        .then((value) => districtRequest(li5.items.cusState))
        .then((value) {
      setState(() {
        dropdownValue2 = li5.items.districtName;
      });
    });
    selectedAvengers = [];
...
//    print("response: ${PlanServiceYearClass.toJson()}");
  ...
  Widget build(BuildContext context) {
    Future<int> uploadImage() async {
      ...
      request.fields['cus_code'] = CusCodeController.text;
      request.fields['cus_id'] = widget.id;
                          ...
                          child: FlatButton(
                            onPressed: () {
                              if (PincodeController.text.length == 6 &&
                                ...
                                else if (CusCodeController.text.length < 6)
                                  showDialog<void>(
                                    context: context,
                                    barrierDismissible: false,
                                    // user must tap button!
...
class BackendService {
  ...
  static Future<List> getSuggestions(String query) async {
    var url;
    if (EmployeeCustomerEditState.dropdownValue == "Vendor") {
      url =
    ...
    } else if (EmployeeCustomerEditState.dropdownValue == "Supervisor") {
      url =
          //'http://35.184.156.126.xip.io/cyberhome/home.php?username=kavin&query=table';
