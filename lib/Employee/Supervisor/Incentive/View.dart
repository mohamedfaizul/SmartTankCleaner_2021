...
import 'package:tankcare/Employee/EmployeeModels/Incentive/View.dart';
import 'package:tankcare/Employee/EmployeeModels/Incentive/View2.dart';
...
class EmployeeIncentiveView extends StatefulWidget {
  EmployeeIncentiveView({Key key, this.id});

  String id;

  @override
  EmployeeIncentiveViewState createState() => EmployeeIncentiveViewState();
}

class EmployeeIncentiveViewState extends State<EmployeeIncentiveView> {
  ...
  IncentiveViewModel li5;
  IncentiveViewModel2 li6;
  States li;
  ...
  Future<http.Response> details(planid) async {
    ...
    var url = String_values.base_url + 'com-details/' + planid;
    print(url);
    ...
    if (response.statusCode == 200) {
      ...
      li5 = IncentiveViewModel.fromJson(json.decode(response.body));
      details1(li5.items.comUtype, li5.items.comUroleId, li5.items.comUid, li5.items.comMonth);
      setState(() {
        DesignationController.text = li5.items.comUtype.toString();
        UserNameController.text = li5.items.comBy.toString();
        DateController.text =   DateFormat('dd/MM/yyyy').format(
            DateTime.parse(li5.items.comMonth.toString()));
  ...
  Future<http.Response> details1(utype,uroleid,uid,month) async {
    setState(() {
      loading = true;
    });
    var url = String_values.base_url + 'com-list?utype=$utype&urole_id=$uroleid&uid=$uid&cmonth=$month';
    print(url);
    var response = await http.get(
      url,
      headers: {
    ...
    if (response.statusCode == 200) {
      ...
      li6 = IncentiveViewModel2.fromJson(json.decode(response.body));
...
//    print("response: ${PlanServiceYearClass.toJson()}");
  ...
  Widget build(BuildContext context) {
    Future<int> uploadImage() async {
      ...
      request.fields['cus_name'] = DesignationController.text;
      request.fields['cus_phone'] = DateController.text;
      request.fields['cus_email'] = UserNameController.text;
...
class BackendService {
  ...
  static Future<List> getSuggestions(String query) async {
    var url;
    if (EmployeeIncentiveViewState.dropdownValue == "Vendor") {
      url =
    ...
    } else if (EmployeeIncentiveViewState.dropdownValue == "Supervisor") {
      url =
      //'http://35.184.156.126.xip.io/cyberhome/home.php?username=kavin&query=table';
