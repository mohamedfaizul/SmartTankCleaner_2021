...
class EmployeePropertyList extends StatefulWidget {
  EmployeePropertyList({Key key, this.propertyid});

  String propertyid;

  @override
  _EmployeePropertyListState createState() => _EmployeePropertyListState();
}

class _EmployeePropertyListState extends State<EmployeePropertyList> {
  bool loading = false;
  bool sort;
        ...
        for (int i = 0; i < li3.list.length; i++)
          if (li3.list[i].title == "Property") {
            for (int j = 0; j < li3.list[i].children.length; j++)
              if (li3.list[i].children[j].title == "Property List") {
