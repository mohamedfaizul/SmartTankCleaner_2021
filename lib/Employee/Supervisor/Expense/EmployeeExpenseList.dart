...
import 'package:tankcare/Employee/EmployeeModels/ExpenseList/ExpenseListModel.dart';
import 'package:tankcare/Employee/Supervisor/Expense/EmployeeAddExpenseAdd.dart';
import 'package:tankcare/Employee/Supervisor/Expense/EmployeeEditExpense.dart';
import 'package:tankcare/Employee/Supervisor/Expense/EmployeeViewExpense.dart';
import 'package:tankcare/VendorModels/MenuModel.dart';
import 'package:tankcare/string_values.dart';

class EmployeeEXPENSEList extends StatefulWidget {
  @override
  EmployeeEXPENSEListState createState() => EmployeeEXPENSEListState();
}

class EmployeeEXPENSEListState extends State<EmployeeEXPENSEList> {
  bool loading = false;

  EmployeeExpenseListModel li;
  ...
  Future<http.Response> postRequest() async {
    ...
    var url = String_values.base_url +
        'expense-list?search=${searchController.text}&page=${page}&limit=10';
    ...
    if (response.statusCode == 200) {
      ...
      li = EmployeeExpenseListModel.fromJson(json.decode(response.body));
          ...
          if (li3.list[i].title == "Expense") {
            if (li3.list[i].children.toString() == "null") {
              ...
              for (int j = 0; j < li3.list[i].children.length; j++)
                if (li3.list[i].children[j].title == "Expense List") {
  ...
  Future<http.Response> delete(planid) async {
    ...
    var url = String_values.base_url + 'expense-delete/' + planid;
    print(String_values.base_url);
    var response = await http.get(
      url,
      headers: {
                            ...
                            (list) => DataRow(cells: [
                              ...
                              DataCell(Center(
                                ...
                                child: Wrap(
                                    ...
                                    DateTime.parse(list.expenseDate.toString())),
                                          textAlign: TextAlign.center)
                                    ]),
                              ))),
                              DataCell(
