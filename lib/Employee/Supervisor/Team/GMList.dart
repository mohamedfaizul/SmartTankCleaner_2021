...
class GMList extends StatefulWidget {
  @override
  GMListState createState() => GMListState();
}

class GMListState extends State<GMList> {
  bool loading = false;
  ...
  Future<http.Response> postRequest() async {
    ...
    var url = String_values.base_url +
        'gm-list?search=${searchController.text}&page=${page}&limit=10';
    ...
    if (response.statusCode == 200) {
      ...
      li = GMListModel.fromJson(json.decode(response.body));
          ...
          if (li3.list[i].title == "Team") {
            if (li3.list[i].children.toString() == "null") {
              ...
              for (int j = 0; j < li3.list[i].children.length; j++)
                if (li3.list[i].children[j].title == "General Manager") {
