...
class EmployeeRemarksView extends StatefulWidget {
  EmployeeRemarksView({Key key, this.id});

  String id;

  @override
  EmployeeRemarksViewState createState() => EmployeeRemarksViewState();
}

class EmployeeRemarksViewState extends State<EmployeeRemarksView> {
          ...
          : SingleChildScrollView(
              child: new Column(
                children: <Widget>[
                  ...
                  //         child: FlatButton(
                  //           onPressed: () {
                  //             if(_typeAheadController.text.length!=0&&_typeAheadController1.text.length!=0&&_typeAheadController2.text.length!=0)
                  //               uploadImage();
                  //             else
                  //             {
                  ...
                  //                   builder: (BuildContext context) {
                  //                     return AlertDialog(
                  //                       title: Text("Remark Type cannot be empty"),
                  //                       actions: <Widget>[
                  //                         TextButton(
                  //                           child: Text('OK'),
                  //                           onPressed: () {
                  ...
                  //                   builder: (BuildContext context) {
                  //                     return AlertDialog(
                  //                       title: Text("Please Choose Service Type"),
                  //                       actions: <Widget>[
                  //                         TextButton(
                  //                           child: Text('OK'),
                  //                           onPressed: () {
...
class BackendService1 {
  ...
  static Future<List> getSuggestions(String query) async {
    ...
    url = String_values.base_url +
        'role-user-list?urole_id=${EmployeeRemarksViewState.roleid}&search=${query}';
    var response = await http.get(
      url,
      headers: {
        "Content-Type": "application/json",
...
class BackendService2 {
  ...
  static Future<List> getSuggestions1(String query) async {
    ...
    url = String_values.base_url +
        'remark-type-mob-search?urole_id=${EmployeeRemarksViewState.roleid}&search=${query}';
    var response = await http.get(
      url,
      headers: {
        "Content-Type": "application/json",
